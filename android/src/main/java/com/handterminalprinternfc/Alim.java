package com.handterminalprinternfc;


public class Alim {

	Boolean NakitAlim  = false;
	String UreticiAdi  = new String();
	String CuzdanNo = new String();
	String Tarih = new String();
	String Agirlik = new String();
	String Fire = new String();
	String NetAgirlik = new String();
	String Tutar = new String();
	String KesintiTutar = new String();
	String NetTutar = new String();
	String Odeme = new String();
	
}
